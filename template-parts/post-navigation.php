        ...
        <?php if ( $prev_post ) : ?>
            <div class="dt-col-lg-6 dt-col-12">
                <a href="<?php echo esc_url( get_permalink( $prev_post ) ); ?>" class="viceunf-post-nav__card viceunf-post-nav__card--prev" rel="prev">
                    <?php
                    $prev_thumb = get_the_post_thumbnail_url( $prev_post, 'medium_large' );
                    if ( $prev_thumb ) : ?>
                        <span class="viceunf-post-nav__img" style="background-image: url('<?php echo esc_url( $prev_thumb ); ?>');" aria-hidden="true"></span>
                    <?php endif; ?>
                    <span class="viceunf-post-nav__overlay"></span>
                    <span class="viceunf-post-nav__content">
        ...
        <?php if ( $next_post ) : ?>
            <div class="dt-col-lg-6 dt-col-12">
                <a href="<?php echo esc_url( get_permalink( $next_post ) ); ?>" class="viceunf-post-nav__card viceunf-post-nav__card--next" rel="next">
                    <?php
                    $next_thumb = get_the_post_thumbnail_url( $next_post, 'medium_large' );
                    if ( $next_thumb ) : ?>
                        <span class="viceunf-post-nav__img" style="background-image: url('<?php echo esc_url( $next_thumb ); ?>');" aria-hidden="true"></span>
                    <?php endif; ?>
                    <span class="viceunf-post-nav__overlay"></span>
                    <span class="viceunf-post-nav__content">
                        <span class="viceunf-post-nav__label">
